...
@app.route('/admin')
def admin():
    return render_template('admin.html', usuarios=usuarios, reembolsos=reembolsos)
...
@app.route('/admin/usuarios/<tipo>')
def get_usuarios(tipo):
    if tipo.lower() == "comisionistas":
        data = [u for u in usuarios if u['type'] == 'Comisionista']
    elif tipo.lower() == "agentes":
        data = [u for u in usuarios if u['type'] == 'Agente Comercial']
    else:
        data = []
    return jsonify(data)

@app.route('/admin/reembolsos')
def get_reembolsos():
